...
const getValueFromGameValue = (gameValue) => {
  switch(gameValue) {
    case 10:
      return 10
    case 4:
...
const calcBid = (hand) => {
  ...
  hand.forEach((card) => {
    ...
    suits[suit].cards.push({ value, gameValue })
  })
  const bestSuit = {
    points: 0,
    suit: '',
...
const takeBotsTurn = (gameData, setBid, pass, deal, playCard) => {
  ...
  if (phase === 'bid') {
    ...
    const cardToPlay = chooseBotCard(gameData)
    playCard(cardToPlay)
  }
};
